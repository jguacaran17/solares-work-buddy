  ...
  return (
    <div className="space-y-3">
      ...
      <div className="glass-card rounded-[10px] overflow-hidden">
        <div className="grid grid-cols-[70px_1fr_40px_50px_50px] gap-1 px-3 py-2 text-[9px] font-bold uppercase"
        ...
        {visiblePartes.map((p, i) => (
          <div key={i} className="grid grid-cols-[70px_1fr_40px_50px_50px] gap-1 px-3 py-2 items-center"
...
const MaquinariaSection = ({ period }: { period: Period }) => {
  ...
  return (
    <div className="space-y-3">
        ...
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={chartData} layout="vertical" barSize={14}>
            ...
            <Bar dataKey="avail" radius={[0, 4, 4, 0]}>
              {chartData.map((d, i) => <Cell key={i} fill={d.avail >= 90 ? "hsl(152,60%,42%)" : d.avail >= 80 ? "hsl(38,78%,52%)" : "hsl(6,65%,46%)"} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="glass-card rounded-[10px] overflow-hidden">
        <div className="grid grid-cols-[1fr_65px_45px_60px] gap-1 px-3 py-2 text-[9px] font-bold uppercase"
          style={{ background: "hsl(var(--g05))", color: "hsl(var(--g5))", borderBottom: "1px solid hsl(var(--g1))" }}>
...
const FlotaSection = ({ period }: { period: Period }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="glass-card rounded-[10px] overflow-hidden">
        <div className="grid grid-cols-[80px_1fr_1fr_60px] gap-1 px-3 py-2 text-[9px] font-bold uppercase"
          style={{ background: "hsl(var(--g05))", color: "hsl(var(--g5))", borderBottom: "1px solid hsl(var(--g1))" }}>
        ...
        {VEHICLES.map((v, i) => (
          <div key={v.plate} className="grid grid-cols-[80px_1fr_1fr_60px] gap-1 px-3 py-2 items-center"
            ...
            <span className="text-[11px] font-mono text-right" style={{ color: "hsl(var(--g7))" }}>{v.kmStart.toLocaleString("es-ES")}</span>
            <span className="text-[11px] font-mono text-right" style={{ color: "hsl(var(--g7))" }}>{v.kmEnd.toLocaleString("es-ES")}</span>
            <div className="text-right">
              ...
              <div className="mt-1 h-1 rounded-full overflow-hidden" style={{ background: "hsl(var(--g1))" }}>
                <div className="h-full rounded-full" style={{ width: `${Math.min(100, (v.kmDay / 100) * 100)}%`, background: v.kmDay > 80 ? "hsl(var(--primary))" : "hsl(var(--g4))" }} />
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <KPI label="Desviación total" value={`${totalDesvPct >= 0 ? '' : ''}${totalDesvPct.toFixed(1)}%`}
          spark={[2, -1, 3, -2, 1, -3, totalDesvPct]}
          sparkColor={totalDesvPct <= 0 ? "hsl(var(--success))" : "hsl(var(--destructive))"} />
        <div className="stat-card flex flex-col gap-0.5">
          <div className="kmi-label">Operarios por tipo</div>
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[8px] font-bold" style={{ background: '#fef3c7', color: '#92400e' }}>D {totalDesp}</span>
            <span className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[8px] font-bold" style={{ background: '#ccfbf1', color: '#115e59' }}>L {totalLocal}</span>
            <span className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[8px] font-bold" style={{ background: '#dbeafe', color: '#1e3a5f' }}>F {totalField}</span>
          </div>
        </div>
      ...
      <div className="glass-card rounded-[10px] overflow-hidden">
        <div
          className="grid grid-cols-[1fr_50px_60px_45px_45px_35px] gap-1 px-3 py-2 text-[9px] font-bold uppercase"
          style={{ background: "hsl(var(--g05))", color: "hsl(var(--g5))", borderBottom: "1px solid hsl(var(--g1))" }}>
        ...
        {data.map((a, i) => {
          ...
          const desvColor = a.desv <= 0 ? '#16a34a' : a.desv < 10 ? '#d97706' : '#dc2626';
          return (
            <div key={a.name}>
              <div
                className="grid grid-cols-[1fr_50px_60px_45px_45px_35px] gap-1 px-3 py-2 items-center cursor-pointer"
                style={{ borderBottom: "1px solid hsl(var(--g1))" }}
                onClick={() => setExpandedActivity(isExpanded ? null : a.name)}
              >
                <div>
                  <div className="text-[11px] font-semibold flex items-center gap-1" style={{ color: "hsl(var(--g8))" }}>
              ...
              <div className="px-3 pb-1">
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "hsl(var(--g1))" }}>
                  <div className="h-full rounded-full transition-all duration-500"
                    style={{ width: `${a.pct}%`, background: a.pct >= 100 ? "hsl(var(--success))" : a.pct >= 75 ? "hsl(var(--primary))" : a.pct >= 50 ? "hsl(var(--warning))" : "hsl(var(--destructive))" }} />
                </div>
              </div>
              {/* Expanded: DESP/LOC/FLD breakdown + subtareas */}
              {isExpanded && (
                <div className="px-3 pb-3 space-y-2" style={{ background: "hsl(var(--g05))" }}>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      <div className="rounded px-2 py-1.5 text-center" style={{ background: '#fef3c7' }}>
                        <div className="text-[8px] font-bold" style={{ color: '#92400e' }}>DESP</div>
                        <div className="text-[11px] font-bold font-mono" style={{ color: '#92400e' }}>{a.desp} op.</div>
                        <div className="text-[9px] font-mono" style={{ color: '#92400e' }}>{(a.despHH / div).toFixed(0)}h</div>
                      </div>
                      <div className="rounded px-2 py-1.5 text-center" style={{ background: '#ccfbf1' }}>
                        <div className="text-[8px] font-bold" style={{ color: '#115e59' }}>LOC</div>
                        <div className="text-[11px] font-bold font-mono" style={{ color: '#115e59' }}>{a.local} op.</div>
                        <div className="text-[9px] font-mono" style={{ color: '#115e59' }}>{(a.localHH / div).toFixed(0)}h</div>
                      </div>
                      <div className="rounded px-2 py-1.5 text-center" style={{ background: '#dbeafe' }}>
                        <div className="text-[8px] font-bold" style={{ color: '#1e3a5f' }}>FLD</div>
                        <div className="text-[11px] font-bold font-mono" style={{ color: '#1e3a5f' }}>{a.field} op.</div>
                        <div className="text-[9px] font-mono" style={{ color: '#1e3a5f' }}>{(a.fieldHH / div).toFixed(0)}h</div>
                      </div>
                    </div>
                  </div>
                  {/* Subtareas */}
                  <div>
                    ...
                    {a.subtareas.map((st, si) => (
                      <div key={si} className="flex justify-between py-1" style={{ borderBottom: si < a.subtareas.length - 1 ? '1px solid hsl(var(--g1))' : 'none' }}>
                        <span className="text-[10px]" style={{ color: "hsl(var(--g7))" }}>{st.name}</span>
                        <span className="text-[10px] font-mono font-bold" style={{ color: "hsl(var(--g8))" }}>{(st.hh / div).toFixed(0)}h</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="grid grid-cols-[1fr_50px_60px_45px_45px_35px] gap-1 px-3 py-2.5 items-center"
          ...
          <span className="text-[10px] font-bold font-mono text-right" style={{ color: totalDesvPct <= 0 ? '#16a34a' : '#dc2626' }}>
      ...
      <div className="flex gap-1 overflow-x-auto pb-1">
        {SECTIONS.map(s => (
          <button key={s.key} onClick={() => setSection(s.key)}
            ...
            style={{ background: section === s.key ? "hsl(var(--secondary))" : "hsl(var(--g05))", color: section === s.key ? "hsl(var(--secondary-foreground))" : "hsl(var(--g7))" }}>
            {s.label}
          </button>
        ))}
      </div>
      <div className="flex gap-1.5">
        {(["hoy", "semana", "mes", "todo"] as Period[]).map(p => (
          <button key={p} onClick={() => setPeriod(p)}
            ...
            style={{ background: period === p ? "hsl(var(--primary))" : "hsl(var(--g1))", color: period === p ? "hsl(var(--primary-foreground))" : "hsl(var(--g6))" }}>
            {PERIOD_LABELS[p]}
          </button>
        ))}
      </div>
